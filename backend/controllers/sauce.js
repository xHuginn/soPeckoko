...
exports.modifySauce = (req, res, next) => {
  ...
  { 
    ...
    // Changer l'image de la manière suivante :
    // req.protocol pour le ""
    // (écrire les détails)
    imageUrl: `${req.protocol}://${req.get('host')}/images/${req.file.filename}`
...
exports.likeSystem = (req, res, next) => {
  // On défini les user, like et sauce
    // const user = req.body.userId;
    const like = req.body.like;

    // On utilise switch pour faire en fonction des cas
        switch(like) {
          // Dans le premier cas, si like = 0
          // case 0: 
          // // On trouve la sauce
          // Sauce.findOne({ _id: req.params.id })
          
          // .then((sauce) => {
          // // Si l'utilisateur est trouvé dans le tableau usersLiked alors :
          //   if(sauce.usersLiked.find(user => user === req.body.userId)) {
          //     Sauce.updateOne({ _id: req.params.id }, { 
          //       // On incrémente 0 (pour enlever le like)
          //       $inc: { likes: -1 },
          //       // On enlève l'user avec pull
          //       $pull: { usersLiked: req.body.userId }
          //      })
          //       .then(() => res.status(200).json({ message: 'Like enlevé !'}))
          //       .catch(error => res.status(400).json({ error }));       
          // }
          // // Si l'utilisateur est trouvé dans le tableau userDisliked alors :
          // if(sauce.usersDisliked.find(user => user === req.body.userId)) {
          //   Sauce.updateOne({ _id: req.params.id }, { 
          //     // On incrémente 0 (pour enlever le dislike)
          //     $inc: { dislike: -1 },
          //     // On enlève l'user avec pull
